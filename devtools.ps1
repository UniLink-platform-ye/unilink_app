...
foreach ($serial in $deviceList) {
    ...
    Invoke-AdbSilent -Serial $serial "shell" "pm" "grant" $PACKAGE_NAME "android.permission.ACCESS_NETWORK_STATE"
    Invoke-AdbSilent -Serial $serial "shell" "pm" "grant" $PACKAGE_NAME "android.permission.ACCESS_WIFI_STATE"
